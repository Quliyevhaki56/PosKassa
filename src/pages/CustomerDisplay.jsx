			...
			<div className='max-w-4xl mx-auto'>
				<div className='bg-white rounded-3xl shadow-2xl overflow-hidden'>
					<div className='bg-blue-600 text-white p-8 text-center'>
						...
						<h1 className='text-5xl font-bold mb-2'>{restaurant?.name || 'Restoran'}</h1>
						{restaurant?.phone && <p className='text-xl text-blue-100'>{restaurant.phone}</p>}
					</div>
					...
					<div className='p-8'>
						{!order || !order.items || order.items.length === 0 ? (
							<div className='text-center py-16'>
								...
								<h2 className='text-4xl font-bold text-gray-800 mb-4'>Xoş gəlmisiniz!</h2>
								<p className='text-2xl text-gray-600'>Sifarişiniz hazırlanır...</p>
							</div>
						) : (
							<>
								<div className='mb-8'>
									<h2 className='text-3xl font-bold text-gray-800 mb-6 text-center'>Sizin sifarişiniz</h2>

									<div className='space-y-4 mb-8'>
										{order.items.map((item, index) => (
											<div key={index} className='flex justify-between items-center bg-gray-50 rounded-lg p-6'>
									...
									<div className='bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-2xl p-8 shadow-xl'>
										<div className='flex justify-between items-center mb-4'>
											<span className='text-2xl font-medium'>Ara cəm:</span>
											<span className='text-3xl font-bold'>
												{order.subtotal.toFixed(2)} {restaurant?.currency || 'AZN'}
											</span>
										</div>

										{order.discount > 0 && (
											<div className='flex justify-between items-center mb-4 text-green-200'>
												<span className='text-2xl font-medium'>Endirim:</span>
												<span className='text-3xl font-bold'>
										...
										<div className='flex justify-between items-center mb-6'>
											<span className='text-2xl font-medium'>ƏDV ({restaurant?.tax_rate || 18}%):</span>
											<span className='text-3xl font-bold'>
										...
										<div className='border-t-2 border-white/30 pt-6'>
											<div className='flex justify-between items-center'>
												<span className='text-3xl font-bold'>ÜMUMI:</span>
												<span className='text-6xl font-bold'>{order.total.toFixed(2)}</span>
											</div>
										</div>
									</div>
								</div>
